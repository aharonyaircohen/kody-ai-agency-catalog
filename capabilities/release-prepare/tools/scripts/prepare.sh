  ...
  case "$prefer" in
    theirs)
      ...
      if [[ -n "$existing" ]]; then
        ...
        emit_goal_report "releasePrExists" "version=${new_version}" "releasePrUrl=${existing}" "releasePr=${existing##*/}"
...
emit_goal_report "releasePrExists" "version=${new_version}" "releasePrUrl=${pr_url}" "releasePr=${pr_url##*/}"
