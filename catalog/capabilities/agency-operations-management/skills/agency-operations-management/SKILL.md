...
description: Operate one active persisted Operation within its exact Workflow and Loop scope.
---
...
4. Validate version `1`, matching `id`, `status: active`, non-empty `intentIds`, non-empty `doesNotOwn`, and at least one owned Workflow or Loop. Refuse malformed or inactive Operations.
5. Read every linked Intent from `<state.path>/intents/<intentId>/intent.json` and require it to be active. Resolve each owned Workflow from `.kody-engine/definitions/workflows/<id>/workflow.json` and each owned Loop from `.kody-engine/definitions/loops/<id>/loop.json` in the consumer repository at its default branch. Refuse unresolved references or model mismatches; never silently drop one. For every Loop, verify its target Workflow or Capability also resolves.
6. Read policy limits, current run state, and the latest `ai-agency-health` evidence only for those owned Workflows and Loops. Treat `doesNotOwn` and linked Intent hard rules as mandatory boundaries.
